...
class BookbankController extends Controller
{
    ...
    public function detail(Request $request, $id) 
    {
        ...
        $accounts = DB::table('accounts')->where('user', $user->id)->orderByDesc("id")->get();
        $bookbank = DB::table('bookbank')->where('id', $id)->first();
        return view('/bookbank/detail', compact("bookbank", "accounts", 'title'));
    }
    ...
    public function updatesettle(Request $request)
    {
        ...
        DB::table('bookbank')->where('id', $request->input('bookbankid'))->update(['accountpay' => $receive, 'settledate' => $settledate]);
        ...
        $daysPassed = (strtotime($settledate) - strtotime($sendDate)) / (60 * 60 * 24);

        // Kiểm tra xem ngày hiện tại có sau ngày gửi và thời hạn không
        if (strtotime($settledate) >= $dueDate) {
            $interest = $bookbank->amount * $bookbank->interest / 1200 * $term ;
        } elseif(strtotime($sendDate) == strtotime($settledate) ) {
            $interest = 0 ;

        } elseif(strtotime($settledate) < $dueDate && $bookbank->payinterest == 'begin' || strtotime($settledate) < $dueDate && $bookbank->payinterest == 'monthlyperiod') {
            $interest = 0 ;
        } else {
